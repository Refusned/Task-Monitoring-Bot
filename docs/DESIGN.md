...
5. Для live-заказов на подписчиков, лайки и просмотры YouTube снимается baseline публичного
   счётчика. Без baseline заказ не создаётся.
6. Самый дешёвый кандидат превращается в `OrderSpec`.
7. Создание заказа идёт через тот же C1-safe путь, что и ручной flow.
8. Сомнительные проверки в live-режиме автоматически отклоняются политикой
...
| Подписки / лайки / просмотры | Baseline + финальный счётчик активности; YouTube subscribers/likes/views через YouTube Data API | `auto_pass`, `needs_human_review`, `fail` |
